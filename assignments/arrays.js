...
var logString = "Car 33 is a " + inventory[32].car_year + " " + inventory[32].car_make + " " + inventory[32].car_model + ".";
console.log(logString);

// ==== Challenge 2 ====
// The dealer needs the information on the last car in their inventory.  What is the make and model of the last car in the inventory?  Log the make and model into the console.
let lastCar = inventory.length;
logString = inventory[lastCar-1].car_make + " " + inventory[lastCar-1].car_model + ".";
console.log(logString);

// ==== Challenge 3 ====
// The marketing team wants the car models listed alphabetically on the website. Sort all the car model names into alphabetical order and log the results in the console
let carModels = inventory.sort(function(carA,carB){
    var carModelA = carA.car_model.toLowerCase();
    var carModelB = carB.car_model.toLowerCase();
...
function makeListOldCars(){
    let getOldCars = [];

    carYears.forEach(function(item,index){
        if(item < 2000){
            getOldCars.push(carYears[index]);
        }
    }); 
...
function getSpecificCars(){
    let specificCars = [];

    inventory.forEach(function(item,index){
        if(item.car_make === "BMW"){
            specificCars.push(inventory[index]);
        }else if(item.car_make === "Audi"){
            specificCars.push(inventory[index]);
        }else{/*do nothing*/}
    });
...
let BMWAndAudi = getSpecificCars();
console.log(JSON.stringify(BMWAndAudi));

//Using .filter to achieve the same result as above

BMWAndAudi = inventory.filter( make=> make.car_make==="BMW"|make.car_make==="Audi");
console.log(JSON.stringify(BMWAndAudi));
